...
class Policy(NeuralNetwork):
    ...
    def evaluate(self, eval_env, n_eval_episodes = 30, max_episode_length = 100):
        ...
        max_episode_length: maximum amount of environment steps within one episode 
            Return:
        mean_reward: mean total reward of all evaluation episodes
        """
...
class Policy_based_RL():
    ...
    def calculate_value_estimate(self, states, rewards):
        """Calculate estimates of the state-action value or total expected return for all states along one trace
        states: states encountered in the trace
        rewards: rewards recieved after taking an action in the trace
            Return:
        value_estimate_per_state: estimate of the state-action value or total expected return for all states along the trace
        """
    ...
    def general_policy_based_algorithm(self, n_timesteps, n_traces, eval_interval):
        ...
        for time in range(int(n_timesteps / (n_traces  * self.trace_length))):
            ...
            policy_loss_per_trace = [] # policy loss of every trace in the batch, loss = gradient 
            value_function_loss_per_trace = [] # value function loss of every trace in the batch, loss = gradient 
            ...
            for _ in range(n_traces):
                ...
                # Calculate loss of the value function network for the actor-critic method
...
if __name__ in "__main__":
    ...
    policy_gradient_variance = False

    parser = argparse.ArgumentParser()
    parser.add_argument("--hyperparameter_tuning", action = "store_true", help = "If given, the hyperparameter tuning results are recreated")
    parser.add_argument("--optimal_performance", action = "store_true", help = "If given, the performances using REINFORCE's best hyperparameters for all methods are recreated")
    parser.add_argument("--policy_gradient_variance", action = "store_true", help = "If given, the experiment comparing the varaince is recreated")
    
    args = parser.parse_args()
    if args.hyperparameter_tuning: hyperparameter_tuning = args.hyperparameter_tuning
    if args.optimal_performance: optimal_performance = args.optimal_performance
    if args.policy_gradient_variance: policy_gradient_variance = args.policy_gradient_variance

    if hyperparameter_tuning:
        # Recreate hyperparameter tuning experiment
        hyperparam_dict = {
        ...
        for param_idx in range(len(hyperparam_dict)):
            for hyperparam in list(hyperparam_dict.keys())[param_idx:param_idx + 1]:
                averaged_runs(hyperparam, 
                            hyperparam_array = hyperparam_dict[hyperparam][0], 
                            hyperparam_label = hyperparam_dict[hyperparam][1], 
                            method = "reinforce")

    elif optimal_performance:
        # Getting the performances of the methods using REINFORCE's best hyperparameters
        averaged_runs(method = "reinforce", best = True)
        averaged_runs(method = "bootstrapping", best = True)
        averaged_runs(method = "baseline_subtraction", best = True)
        averaged_runs(method = "bootstrapping_baseline_subtraction", best = True)

    elif policy_gradient_variance:
